  ...
  def apply (algo: String, graph: Graph, maxIters: Int, 
    ...
    if (useActors) {
      ...
      if (resultList.size > 0) {
        ...
        // Logger isn't working right now, so checking this way.
        println(Constants.GetPrecisionString + " " + res(Constants.GetPrecisionString))
        println(Constants.GetMRRString + " " + res(Constants.GetMRRString))
      }
